...
CREATE OR REPLACE TABLE DIMST (
    ...
    --CONSTRAINT FK_DIMST_DITBL FOREIGN KEY (numero_tabla)
     --   REFERENCES DITBL (numero_tabla)
)
RCDFMT DIMSTR;

RENAME TABLE DIMST
    TO DIMST_TABLE FOR SYSTEM NAME DIMST;

COMMENT ON TABLE DIMST IS
    'Maestro de Inventario de Documentos - Modulo 14 Manejo de Documentos Taller IBM i';

LABEL ON TABLE DIMST IS
    'Inventario de Documentos';

COMMENT ON COLUMN DIMST.tipo_cuenta           IS 'Tipo de cuenta u operacion a la que pertenece el documento inventariado; parte de la PK';
COMMENT ON COLUMN DIMST.numero_tabla          IS 'Numero de tabla de tipos de documentos; FK a DITBL.numero_tabla; parte de la PK';
COMMENT ON COLUMN DIMST.secuencia             IS 'Numero de secuencia del documento dentro del inventario por tipo de cuenta; parte de la PK';
COMMENT ON COLUMN DIMST.tipo_documento        IS 'Codigo del tipo de documento recibido segun catalogo (IDENTIDAD, CONTRATO, etc.)';
COMMENT ON COLUMN DIMST.descripcion_documento IS 'Descripcion del documento especifico recibido para el expediente';
COMMENT ON COLUMN DIMST.obligatorio           IS 'Indica si el documento es obligatorio para esta operacion: S=Si, N=No';
COMMENT ON COLUMN DIMST.fecha_recepcion       IS 'Fecha en que se recibio fisicamente el documento del cliente';
COMMENT ON COLUMN DIMST.fecha_vencimiento     IS 'Fecha de vencimiento o expiracion del documento recibido';
COMMENT ON COLUMN DIMST.observaciones         IS 'Notas del gestor documental sobre el estado o condicion del documento';
COMMENT ON COLUMN DIMST.usuario_creacion      IS 'Usuario del sistema que registro el documento en el inventario';
COMMENT ON COLUMN DIMST.usuario_actualizacion IS 'Usuario del sistema que realizo la ultima modificacion al registro';
COMMENT ON COLUMN DIMST.version_registro      IS 'Contador de versiones para control de concurrencia optimista';
COMMENT ON COLUMN DIMST.estado_registro       IS 'Estado logico del documento en el inventario: A=Activo, I=Inactivo';
COMMENT ON COLUMN DIMST.created_at            IS 'Fecha y hora exacta de creacion del registro en la base de datos';
COMMENT ON COLUMN DIMST.updated_at            IS 'Fecha y hora de la ultima actualizacion registrada en la base de datos';
...
CREATE INDEX IDX_DIMST_C  ON DIMST (created_at);
CREATE INDEX IDX_DIMST_TC ON DIMST (tipo_cuenta, numero_tabla);
